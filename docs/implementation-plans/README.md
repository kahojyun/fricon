# Implementation Plans

## Status

Sentinel. No active milestone implementation plans.

## Rule

Do not create milestone plans while product, architecture, specs, or
required ADR boundaries are still unsettled.

Implementation plans may be recreated only after a derived spec has accepted
requirements, design, traceability, and validation. A plan should sequence
implementation work; it must not introduce product scope, product terminology,
architecture decisions, or non-goals on its own.

## Recreation Criteria

Before adding an `M#-*` plan, confirm:

- the relevant upstream baseline is accepted or explicitly marked with open
  interview questions
- the relevant spec is accepted for implementation planning
- required ADRs are accepted or listed as blocking upstream decisions
- migration, compatibility, and validation posture are clear upstream
