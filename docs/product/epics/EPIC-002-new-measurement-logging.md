...
measurements: declare measured data, append values and traces from Python,
watch live monitor plots, and reopen it later without manual file/folder
discipline.
...
- Dedicated trace-valued records for VNA-like measurements, including multiple
  traces within one outer sweep record.
- First-class complex values so magnitude/phase or I/Q views can be derived
  from recorded semantics instead of channel naming conventions.
...
measurement scripts to migrate without a full old-system importer. The first
case-study pressure is a VNA S21 scan with outer DC-voltage and VNA-power
sweeps, trace-valued S21 outputs, IQ/readout variants, and occasional minimizer
step records.
