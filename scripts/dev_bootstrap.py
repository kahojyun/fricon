...
class ProjectPaths:
    ...
    cargo_entrypoint_path: Path
    cargo_shared_path: Path
    cargo_worktree_path: Path
    cargo_user_path: Path
    ...
    def discover(cls) -> ProjectPaths:
        ...
        return cls(
            ...
            cargo_entrypoint_path=cargo_directory / "config.toml",
            cargo_shared_path=cargo_directory / "config.shared.toml",
            cargo_worktree_path=cargo_directory / "config.worktree.toml",
            cargo_user_path=cargo_directory / "config.user.toml",
        )


def bootstrap_checkout(paths: ProjectPaths) -> tuple[Path, Path]:
...
def resolve_pyo3_python(root: Path) -> Path:
    ...
    discovered_python = shutil.which("python3") or shutil.which("python")
    if discovered_python is not None:
        return Path(discovered_python).resolve()

    return Path(sys.executable).resolve()


def resolve_shared_target_dir(root: Path) -> Path:
    repo_target_dir = resolve_repo_target_dir(root)
    candidates = [
        repo_target_dir,
        resolve_user_cache_dir() / "fricon" / "cargo-target",
        Path.home() / ".cargo" / "fricon-target",
        Path(tempfile.gettempdir()) / "fricon" / "cargo-target",
    ]
    ...
    raise PermissionError("Unable to create a shared Cargo target directory")


def resolve_repo_target_dir(root: Path) -> Path:
    try:
...
def write_worktree_cargo_config(
    path: Path, python_path: Path, target_dir: Path
) -> None:
    ...
    path.write_text(
        (
            ...
            "[env]\n"
            f'PYO3_PYTHON = "{python_path.resolve().as_posix()}"\n\n'
            "[build]\n"
