...
def setup_env(args: argparse.Namespace) -> bool:
    ...
    return run_command(
        [sys.executable, str(setup_script)], description="Running setup script"
    )
...
def build_python(args: argparse.Namespace) -> bool:
    ...
    success &= run_command(
        ["uv", "sync", "--dev"], cwd=root, description="Installing Python dependencies"
    )
    ...
    success &= run_command(
        ...
        cwd=root,
        description="Building Python extension",
    )
...
def test_frontend(args: argparse.Namespace) -> bool:
    ...
    return run_command(
        ["pnpm", "run", "test"], cwd=frontend_dir, description="Running frontend tests"
    )
...
def lint_all(args: argparse.Namespace) -> bool:
    ...
    success &= run_command(
        ["cargo", "fmt", "--all", "--check"],
        cwd=root,
        description="Checking Rust formatting",
    )
    success &= run_command(
        ["cargo", "clippy", "--all-targets", "--all-features"],
        cwd=root,
        description="Running Rust linter",
    )
    ...
    success &= run_command(
        ["uv", "run", "ruff", "check", "."],
        cwd=root,
        description="Running Python linter",
    )
    success &= run_command(
        ["uv", "run", "ruff", "format", "--check", "."],
        cwd=root,
        description="Checking Python formatting",
    )

    # Frontend
    success &= run_command(
        ["pnpm", "run", "lint"], cwd=frontend_dir, description="Running frontend linter"
    )
...
def fix_all(args: argparse.Namespace) -> bool:
    ...
    # Rust
    success &= run_command(
        ["cargo", "fmt", "--all"], cwd=root, description="Formatting Rust code"
    )
    ...
    success &= run_command(
        ...
        cwd=root,
        description="Fixing Python issues",
    )
    success &= run_command(
        ["uv", "run", "ruff", "format", "."],
        cwd=root,
...
def clean_all(args: argparse.Namespace) -> bool:
    ...
    # Rust
    success &= run_command(
        ["cargo", "clean"], cwd=root, description="Cleaning Rust artifacts"
    )
...
def main() -> NoReturn:
    ...
    build_subparsers = build_parser.add_subparsers(
        dest="build_target", help="Build targets"
    )
    ...
    python_parser = build_subparsers.add_parser(
        "python", help="Build Python components"
    )
    python_parser.set_defaults(func=build_python)

    frontend_parser = build_subparsers.add_parser(
        "frontend", help="Build frontend components"
    )
    frontend_parser.add_argument(
        "--dev", action="store_true", help="Start development server"
    )
    ...
    test_subparsers = test_parser.add_subparsers(
        dest="test_target", help="Test targets"
    )
    ...
    test_python_parser.add_argument(
        "--coverage", action="store_true", help="Generate coverage report"
    )
    test_python_parser.set_defaults(func=test_python)

    test_frontend_parser = test_subparsers.add_parser(
        "frontend", help="Run frontend tests"
    )
