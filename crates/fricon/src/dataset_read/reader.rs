...
use crate::{
    DatasetDataType, DatasetSchema, dataset_catalog::DatasetCatalogError,
    dataset_ingest::WriteSessionHandle, dataset_read::SelectOptions, dataset_schema::DatasetError,
    storage::ChunkReader,
};
...
impl DatasetSource {
    ...
    fn select_data(
    ...
    ) -> Result<(SchemaRef, Vec<RecordBatch>), DatasetCatalogError> {
        ...
        match self {
            ...
            Self::File(reader) => select_data(
                ...
                reader.schema().ok_or(DatasetCatalogError::EmptyDataset)?,
                index_filters,
                selected_columns,
            )
            .map_err(Into::into),
...
impl DatasetReader {
    pub(crate) fn from_handle(source: WriteSessionHandle) -> Result<Self, DatasetCatalogError> {
    ...
    pub(crate) fn open_dir(path: PathBuf) -> Result<Self, DatasetCatalogError> {
        let mut reader = ChunkReader::new(path, None);
        reader.read_all()?;
        let arrow_schema = reader
            .schema()
            .ok_or(DatasetCatalogError::EmptyDataset)?
            .clone();
        let schema = arrow_schema.as_ref().try_into()?;
        Ok(Self {
