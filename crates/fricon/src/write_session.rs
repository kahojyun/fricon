...
use crate::{
    live::{LiveDataset, LiveDatasetWriter},
    utils::read_ipc_file_mmap,
};

/// High-level write session combining a `BackgroundWriter` with a
/// `LiveDataset`.
///
...
impl WriteSession {
    pub fn new(tracker: &TaskTracker, dir_path: impl Into<PathBuf>, schema: SchemaRef) -> Self {
        ...
        tracker.spawn(async move {
            // Single chunk completion expected in current design; loop if future needs
            // multiple.
