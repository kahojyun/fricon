...
use crate::{
    dataset_catalog::DatasetCatalogError, dataset_ingest::storage::InProgressTable,
    storage::ChunkWriter,
};
...
impl WriteSession {
    ...
    pub(super) fn write(&mut self, batch: RecordBatch) -> Result<(), DatasetCatalogError> {
    ...
    pub(super) fn finish(self) -> Result<(), DatasetCatalogError> {
        self.in_progress_table_mut().mark_complete();
        self.writer.finish()?;
        Ok(())
    }

    pub(super) fn abort(self) -> Result<(), DatasetCatalogError> {
        self.writer.finish()?;
        Ok(())
    }
