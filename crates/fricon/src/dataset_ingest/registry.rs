...
use crate::{
    dataset_catalog::DatasetCatalogError,
    dataset_ingest::{WriteSessionHandle, session::WriteSession},
};
...
impl WriteSessionGuard {
    ...
    pub(crate) fn commit_session(mut self) -> Result<(), DatasetCatalogError> {
    ...
    pub(crate) fn abort_session(mut self) -> Result<(), DatasetCatalogError> {
        if let Some(session) = self.session.take() {
            session.abort()?;
        }
