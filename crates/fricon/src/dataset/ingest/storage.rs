...
use crate::dataset::{ingest::IngestError, schema::ChunkedTable, storage::ChunkReader};
...
impl InProgressTable {
    ...
    pub(super) fn push(&mut self, batch: RecordBatch) -> Result<(), IngestError> {
        self.in_memory.push_back(batch)?;
        Ok(())
    }

    pub(super) fn continue_read_chunks(&mut self) -> Result<(), IngestError> {
