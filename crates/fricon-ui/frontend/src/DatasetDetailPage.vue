...
<template>
  <div class="flex h-full flex-col gap-3 overflow-auto p-3">
    <div v-if="isLoading" class="text-surface-500 text-sm">
      Loading dataset...
    </div>

    <div v-else-if="detail" class="flex flex-1 flex-col overflow-hidden">
      <Tabs
      ...
      >
        ...
        <TabPanels class="flex flex-1 flex-col overflow-hidden">
          <TabPanel value="details" class="h-full overflow-auto">
            <div class="flex flex-col gap-3 p-3 pt-3">
              <div
                class="grid gap-3 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]"
              >
                <div
                  class="border-surface-200 flex flex-col gap-3 rounded border p-3"
                >
                  <div class="flex items-center justify-between gap-2">
                    <h2 class="text-base font-semibold">Dataset Details</h2>
                    <Button
                      label="Save"
                      icon="pi pi-save"
                  ...
                  <div v-else-if="successMessage">
                    <Message severity="success" :closable="false" size="small">
                      {{ successMessage }}
                    </Message>
                  </div>

                  <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium" for="dataset-name"
                      >Name</label
                    >
                    <InputText
                      id="dataset-name"
                  ...
                  <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium" for="dataset-description"
                      >Description</label
                    >
                    <Textarea
                  ...
                  <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium" for="dataset-tags"
                      >Tags</label
                    >
                    <InputText
                      id="dataset-tags"
                ...
                >
                  ...
                  <div class="flex flex-col gap-2">
                    <div class="text-sm font-medium">Current Tags</div>
                    <div class="flex flex-wrap gap-1">
                      <Tag v-for="tag in detail.tags" :key="tag">{{ tag }}</Tag>
                      <span
              ...
              <div class="border-surface-200 rounded border p-3">
                <div class="mb-2 flex items-center justify-between">
                  <h3 class="text-sm font-semibold">Columns</h3>
                  <span class="text-surface-500 text-xs"
                    >{{ detail.columns.length }} columns</span
                  >
                </div>
                <DataTable
          ...
          <TabPanel value="charts" class="flex flex-1 flex-col overflow-hidden">
            <div class="flex flex-1 flex-col overflow-hidden p-3 pt-3">
    ...
    <div v-else class="text-surface-500 text-sm">Dataset not found.</div>
  </div>
</template>
