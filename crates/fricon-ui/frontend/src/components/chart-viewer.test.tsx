...
describe("ChartViewer", () => {
  ...
  it("falls back to unfiltered chart request when filter table query fails", async () => {
    let chartCallCount = 0;
    mockIPC((cmd) => {
      if (cmd === "dataset_detail") {
        return {
          ...
          createdAt: new Date().toISOString(),
          columns: [
            { name: "idx", isComplex: false, isTrace: false, isIndex: true },
            {
              name: "trace_variable_voltage_v",
              isComplex: false,
              isTrace: true,
              isIndex: false,
            },
      ...
      if (cmd === "get_filter_table_data") {
        throw new Error("filter table unavailable");
      }
      if (cmd === "dataset_chart_data") {
        chartCallCount += 1;
        return {
          type: "line",
          xName: "trace_variable_voltage_v - X",
          series: [
            {
              name: "trace_variable_voltage_v",
              data: [
